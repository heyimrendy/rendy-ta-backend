...
class jalanSpellCheck:
    ...
    def correctSpelling(self, text):
        sc = spellCheck()
        return (
            sc.correct(text) if text not in self.__katadasar else text
        )  # for t in text.split()
...
class slangWordCorrect:
    def jalan(self, text):
        ...
        reformed = [
            listWord[word] if word in listWord else word for word in sentence_list
        ]
...
def create_connection(db_file: str):
    ...
    try:
        ...
        print(f"Error create sqlite3 connection: {e}")
...
def test(task_id):
    ...
    joy_feel = read_dataset(get_full_path("dataset/cf/pp/filter/joy.txt"), "joy")
    ...
    anger_feel = read_dataset(get_full_path("dataset/cf/pp/filter/anger.txt"), "anger")
    fear_feel = read_dataset(get_full_path("dataset/cf/pp/filter/fear.txt"), "fear")
    surprise_feel = read_dataset(
        get_full_path("dataset/cf/pp/filter/surpriseExtra.txt"), "surprise"
    )

    dataku = []
    for (words, sentiment) in (
        joy_feel + disgust_feel + sadness_feel + anger_feel + fear_feel + surprise_feel
    ):
                ...
                if each_video["message"] != "":
                    ...
                    if kata != "":
                        ...
                        comments_data = {
                            ...
                            "angry_count": each_video["angry"]["summary"][
                                "total_count"
                            ],
                        }
